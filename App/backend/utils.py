...
def call_generation(temperature, timesig, numOfBars, TransEncoders, model, nnModel, enc_list):
  ...
  if model == 'transformer':
    allChords, allDurs, allMels = generate_chord_durs_ev_trans(nnModel, enc_list,
                                                               timesig, temperature, numOfBars, TransEncoders,
                                                               enc_seq_length,
                                                               dec_seq_length)
  else:  # LSTM
...
def chords_inf_model_ev(model_tr, LSTM_dim):
  ...
  decoder_model = tf.keras.models.Model(
    [dec_inputs] + decoder1_states_inputs + decoder2_states_inputs + decoder3_states_inputs,
...
def chord_trans_ev_model(enc_vocab, dec_vocab):
  ...
  encoder = Encoder(enc_vocab + 1, num_layers=num_layers, d_model=d_model, num_heads=num_heads, dff=dff,
                    dropout=dropout_rate)
  decoder = Decoder(dec_vocab + 1, num_layers=num_layers, d_model=d_model * 4, num_heads=num_heads, dff=dff,
                    dropout=dropout_rate)
